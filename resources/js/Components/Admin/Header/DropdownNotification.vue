...
const props = defineProps({
    ...
    notifying: { type: Number },
});
onClickOutside(target, () => {
    dropdownOpen.value = false;
});

const emit = defineEmits(["handle-link"]);
const handleLink = (url, notification_id) => {
    dropdownOpen.value = false;
    router.get(url);
    emit("handle-link", notification_id);
};
</script>
...
<template>
    <li class="relative" ref="target">
        ...
        <div
            v-show="dropdownOpen"
            class="absolute w-[90vw] ltr:right-[-20vw] ltr:xsm:right-0 rtl:left-[-20vw] rtl:xsm:left-0 xsm:w-[70vw] mt-2.5 flex h-90 flex-col rounded-sm border border-stroke bg-white shadow-default sm:w-80"
        >
            ...
            >
                <template v-for="(item, index) in notifications" :key="index">
                    <li>
                        <NotificationItem
                            @click="handleLink(item.data.url, item.id)"
                            :notification="item"
                        />
                    </li>
                </template>
            </ul>
            <p v-else class="text-center text-sm p-4">
                {{ $t("dropdown.no_notifications") }}
            </p>

            <Link
                @click="dropdownOpen = false"
                :href="route('notifications.index')"
                class="mt-auto block rounded lg p-2 bg-slate-500 text-slate-200 hover:underline text-center"
            >
                {{ $t("dropdown.see_all") }}
            </Link>
        </div>
