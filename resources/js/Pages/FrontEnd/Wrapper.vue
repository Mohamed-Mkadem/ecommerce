...
const increaseQuantity = () => {
  if (selectedVariant.value.type === "product") {
    totalQuantity.value = totalQuantity.value + 0.5;
  }
  if (selectedVariant.value.type === "pack") {
    totalQuantity.value = totalQuantity.value + 1;
  }
};

const decreaseQuantity = () => {
  if (selectedVariant.value.type === "product" && totalQuantity.value > 0.5) {
    totalQuantity.value = totalQuantity.value - 0.5;
  }
  if (selectedVariant.value.type === "pack" && totalQuantity.value > 1) {
    totalQuantity.value = totalQuantity.value - 1;
  }
};
...
const total = computed(() => {
  return (parseFloat(subtotal.value) + parseFloat(shippingCost.value)).toFixed(
    3,
  );
});
      ...
      >
        ...
        >
          <div>
            <img
              :src="
                currentImage?.original_url ?? selectedVariant.main_image_url
              "
        ...
        <div class="mt-8 lg:mt-0">
          <!-- Title -->
          <h1 class="text-3xl text-primary font-bold mb-4 tracking-tight">
            {{ wrapper.title }}
          </h1>
          ...
          <form @submit.prevent="submitForm" class="space-y-4">
            ...
            <!-- Phone Number -->
            <div>
              <InputLabel for="wrapper-phone" class="text-neutral-500">
                ...
                >
                  ...
                  <div class="flex-1 text-right">
                    <p
                      ...
                      :class="{
                        'text-sky-900': selectedVariant.id === variant.id,
                      }"
                    >
                      {{ variant.name }}
                    </p>
                    <p
                      ...
                      :class="{
                        'text-sky-800': selectedVariant.id === variant.id,
                      }"
                    >
                  ...
                  >
                    <i class="ri-check-line text-base text-white"></i>
                  </div>
                  <!-- Variant Image -->
            ...
            <div class="flex gap-3">
              ...
              <button
                ...
                :class="[
                  ...
                  {
                    'opacity-50 cursor-not-allowed': loadingState,
                  },
                ]"
          ...
          >
            <div class="flex justify-between items-center gap-2 mb-3">
              <p class="text-neutral-600">{{ $t("Sub Total") }}:</p>
            ...
            <div class="flex justify-between items-center gap-2 mb-3">
              <p class="text-neutral-600">
                ...
                <template v-if="form.state">
                  ({{ getLocalizedStateName(form.state.id) }}) </template
                >:
              </p>
              <p class="text-neutral-700 font-semibold">
                {{ shippingCost }}
            ...
            >
              <p class="text-lg font-bold text-sky-900">{{ $t("Total") }}:</p>
