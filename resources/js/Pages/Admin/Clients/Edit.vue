...
const props = defineProps(["states", "client"]);
const toast = useToast();
const locationSelector = useLocationSelector();
onMounted(async () => {
    if (props.client.locality_id) {
        await locationSelector.getCities(props.client.state_id);
        await locationSelector.getLocalities(props.client.city_id);
    }
});
const form = useForm({
    ...
    state: props.client.state_id,
    city: props.client.city_id,
    locality: props.client.locality_id,
});
...
function submitForm() {
    form.patch(route("clients.update", props.client), {
        onSuccess: () => {
            let message = trans("Client.updated_successfully");
    ...
    >
        ...
        <form @submit.prevent="submitForm">
            <div class="md:flex md:flex-row gap-4">
                ...
                <div class="mt-4 w-full">
                    <InputLabel for="value" :value="$t('Phone Number')" />
